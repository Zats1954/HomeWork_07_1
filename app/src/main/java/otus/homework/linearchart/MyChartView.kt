...
//    var _clickSector = MutableLiveData<Int>(-1)
    ...
    init {
        ...
        timeWidth = chartModel.maxTime - chartModel.minTime
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        try {
            scaleAmount = (stopY - startY) / chartModel.maxAmount
        } catch (e: NullPointerException) {
            Toast.makeText(context, "Нет данных ", Toast.LENGTH_LONG).show()
            scaleAmount = 0f
        }

        chartModel.grafData.forEach { t, u ->
            System.out.println("t ${convertLongToTime(t)} u $u")
            graf.add(PointF((t - chartModel.minTime) * scaleTime, u * scaleAmount))
        }
...
// Пределы значений по Y
        ...
        canvas.drawText(
            chartModel.maxAmount.toString(),
            startX - (chartModel.maxAmount.toString().length - 1) * pChart.textSize,
            startY + 30f, pChart
        )

// Пределы значений по X
        val startTime = convertLongToTime(chartModel.minTime)
        val stopTime = convertLongToTime(chartModel.maxTime)
        ...
        System.out.println(" textSize ${chartModel.maxAmount.toString().length}")
